...
export default function CameraScanModal({ onClose, onComplete }: CameraScanModalProps) {
  ...
  const capturePhoto = () => {
    ...
    const newPage: ScanPage = { id: Date.now(), imageData };
  ...
  const handleCrop = () => {
    ...
    img.onload = () => {
      ...
      setPages((prev) =>
        prev.map((p) => (p.id === page.id ? { ...p, cropped } : p))
      );
      setIsCropping(false);
    };
  ...
  const handleConvert = () => {
    ...
    const mimeType =
      outputFormat === "png" ? "image/png" : outputFormat === "jpg" ? "image/jpeg" : "application/pdf";
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:opacity-90"
                    >
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {pages.map((page, i) => (
                      <div key={page.id} className="relative shrink-0">
                        ...
                        />
                        <span className="absolute bottom-1 left-1 text-xs bg-foreground/70 text-background px-1.5 py-0.5 rounded font-medium">
                          {i + 1}
                        </span>
                      </div>
                    ))}
                  ...
                  {isCropping ? (
                    <div className="relative">
                      ...
                      <div className="flex gap-2 mt-3">
                        <button onClick={handleCrop} className="flex-1 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium">
                          Terapkan Crop
                        </button>
                        <button onClick={() => setIsCropping(false)} className="px-4 py-2 rounded-lg border border-input text-sm">
                          Batal
                        </button>
                      </div>
                    </div>
              ...
              {!isCropping && (
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-input text-sm hover:bg-muted"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-input text-sm hover:bg-destructive/10 text-destructive"
                  >
              ...
              {!isCropping && pages.length > 0 && (
                <div className="p-4 rounded-xl border border-border bg-muted/30 space-y-3">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-primary text-primary-foreground font-semibold hover:opacity-90 transition-opacity"
                  >
