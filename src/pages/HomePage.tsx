    ...
    <div className="min-h-screen bg-background">
      ...
      <nav className="sticky top-0 z-40 bg-card/95 backdrop-blur border-b border-border">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-3">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/login")} className="px-5 py-2 rounded-lg border border-input text-sm font-medium hover:bg-muted transition-colors text-foreground">
              Login
            </button>
            <button onClick={() => navigate("/signup")} className="px-5 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90 transition-opacity">
              Sign Up
            </button>
          </div>
        </div>
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-24 md:py-32">
          <div className="max-w-2xl">
            ...
            <p className="text-lg text-primary-foreground/80 mb-8 leading-relaxed">
              SAKURA (Secure Archiving and Keeping of Unified Records for Administration) adalah sistem manajemen arsip digital yang dirancang khusus untuk pengelolaan ijazah dan dokumen akademik secara aman, terstruktur, dan efisien.
            </p>
            <div className="flex gap-3">
              <button onClick={() => navigate("/login")} className="px-8 py-3 rounded-lg bg-primary-foreground text-primary font-bold hover:opacity-90 transition-opacity">
                Masuk ke Sistem
              </button>
              <button onClick={() => navigate("/signup")} className="px-8 py-3 rounded-lg border-2 border-primary-foreground/30 text-primary-foreground font-semibold hover:bg-primary-foreground/10 transition-colors">
                Daftar Akun
              </button>
            </div>
          </div>
